function Plot_NEW_winter_and_summern()
isupdata=false;
if isupdata
...
for i=1:obj.Data.number_of_feeder
    h1 = plot(time,(AA(i,:)*[obj.PevT;obj.PbuyT]) , ...
...
time=0.5:0.5:obj.Data.T/2;  
for i=1:obj.Data.number_of_feeder
    h2 = plot(time,(AA(i,:)*[obj.PevT;obj.PbuyT]) , ...
        'LineStyle', linestyle{2}, 'Color', colors{i}, 'LineWidth', 2);
end

% 只保留蓝色曲线的图例，且不带数字
% 蓝色曲线是 colors{1}，即 i=1 的曲线
legend([h1, h2], {'Summer', 'Winter'}, 'Location', 'best');
% legend([h1(1)], {'With constraint'}, 'Location', 'best');

hold off;
xlabel('Time (h)')
ylabel('Percentage')
title('Powers  of 5 feeders for Summer and Winter with  capacity constraints')
box on
